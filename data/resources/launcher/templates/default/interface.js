...
if (typeof window.__ADMIN_MODE__ === 'undefined') {
    window.__ADMIN_MODE__ = false;
}

const applyAdminModeFlag = (enabled) => {
    const value = !!enabled;
    window.__ADMIN_MODE__ = value;
    try {
        document.body?.classList.toggle('admin-mode-enabled', value);
    } catch (_) {
        /* ignore */
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const isAdminRoute = path.endsWith('/admin-dashboard.html') || path.endsWith('/launcher-editor.html') || path.endsWith('/create.html');
    if (!isAdminRoute) {
        (async () => {
            try {
                if (typeof window.go?.main?.App?.LeaveAdminView === 'function') {
                    await window.go.main.App.LeaveAdminView();
                }
            } catch (_) {
                /* ignore */
            }
    ...
    function setupUpdateListener() {
        ...
        window.runtime.EventsOn('remoteConfigLoaded', (remoteConfig) => {
            console.log('[Interface] Получена удаленная конфигурация:', remoteConfig);
                        ...
                        mainBtn.addEventListener('click', async () => {
                            ...
                            try {
                                if (!isNaN(appId) && window.go?.main?.App?.CheckForUpdatesAndLaunchGameByID) {
                                    ...
                                    if (result === 'UPDATING') {
                                        showAlert && showAlert({ type: 'info', message: (window.i18n?.t ? window.i18n.t('checkingUpdates') : 'Проверка обновлений...') });
                                    }
    ...
    // Функция для входа в полноэкранный режим при переходе в админ-панель
    const enterAdminFullscreen = async () => {
        try {
            if (window.go?.main?.App?.EnterAdminView) {
                await window.go.main.App.EnterAdminView();
                console.log('[Admin] Вызван метод EnterAdminView для полноэкранного режима');
            }
        } catch (err) {
            console.warn('[Admin] Ошибка при вызове EnterAdminView:', err);
        }
    };
    ...
    window.addEventListener('keydown', async (e) => {
        ...
        const isEditable = (active && (active.isContentEditable || tagName === 'input' || tagName === 'textarea' || tagName === 'select'));

        if (isEditable && (e.key === 'F1' || e.key === 'F2')) {
            e.preventDefault();
            active.blur();
        }
        ...
        if (window.runtime && typeof window.runtime.EventsOn === 'function') {
            try {
                window.runtime.EventsOn('app:ready', (payload) => {
                    ...
                    if (payload && typeof payload.adminMode === 'boolean') {
                        applyAdminModeFlag(payload.adminMode);
                    }
